...
$casper->start('http://www.ndtv.com/latest?pfrom=home-topnavigation2016');
$casper->capture(
    array(
        'top' => 0,
        'left' => 0,
...
$Header = $xpath->query("//div[@class='nstory_header']");
$detail=$xpath->query("//div[@class='nstory_intro']");
$image = $xpath->query("//div[@class='new_storylising_img']/a/img/@src");
...
foreach($Header as $NodeHeader) 
	...
	for($i=0;$i<$len;$i++)
	{
		...
		$sql="insert into newscollection(title,description,image,site) values ('$a','$b','$c','$site')";
	$r=mysql_query($sql);
	if($r)
	{
		echo "success";
