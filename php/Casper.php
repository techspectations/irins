...
class Casper
{
    ...
    private $_TAG_POPUP_HTML = '[POPUP_HTML]';
    ...
    private $_currentUrl = '';
    private $_userAgent = 'Mozilla/5.0 (Windows NT 6.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/51.0.2704.103 Safari/537.36';
...
//****************************************************************EXTERNAL FUNCTIONS*******************************************
	...
	public function extraFunction($functions)
    {
...
//////////////////////////////////////////////////////////////////////////////////////
    ...
    public function newWindowClick($selector,$segment)
    {
        $fragment = <<<FRAGMENT
    casper.then(function() {
    this.click('$selector');
    ...
    this.echo('search popup ' + this.popups.findByRegExp(/$segment/));
    });	
	this.waitForPopup(/$segment/, function(){
    this.echo('new window opened');
    });

	 this.withPopup(/$segment/, function(popup){
	 this.echo('popup list entries - '+this.popups.list());
    });
    });
...
//**************************************************************END ERROR HANDLER*************************************
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//***************************************************CAPTURE SELECTOR XPATH FUNCTION**********************************

    public function captureSelectorXpath($selector, $filename)
    {
        $fragment = <<<FRAGMENT
casper.then(function() {
      this.captureSelector(x('$selector'), $filename);
});
...
FRAGMENT;
    ...
    private function _processOutput()
    {
        foreach ($this->getOutput() as $outputLine) {
            ...
            if (strpos($outputLine, $this->_TAG_POPUP_HTML) !== false) {
                $this->_popup_html = str_replace($this->_TAG_POPUP_HTML, '', $outputLine);
            }
